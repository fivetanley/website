...
### RESTAdapter.prototype.findMany changed behavhior and method signature
...
Thanks to [Jeff Lage][jeff-lage], the `DS.EmbeddedRecordsMixin` was extracted
out of `DS.ActiveModelSerializer` in Ember Data v1.0.0-beta.8 so that users of
both the `RESTSerializer` and `ActiveModelSerializer` could easily serialize and
deserialize relationships. To use the code in your app, you can include the
...
a bit faster when used with Ember Data. We'd also like you to try out Ember 1.8
...
[jeff-lage]: https://github.com/jefflage 
